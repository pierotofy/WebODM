...
_("Set the units of the PDF report. By default the vertical units of the coordinate reference system are used. Can be one of: %(choices)s. Default: %(default)s");
...
_("When processing multispectral datasets, automatically align the images for each band. If the images have been postprocessed and are already aligned, use this option. Default: %(default)s");
...
_("Skips dense reconstruction and 3D model generation. It generates an orthophoto directly from the sparse reconstruction. On flat terrain with no objects/structures, turn this on to save time. Default: %(default)s");
...
_("Choose the structure from motion algorithm. For aerial datasets, if camera GPS positions and angles are available, triangulation can be faster. Planar is deprecated and will be removed in a future version. Can be one of: %(choices)s. Default: %(default)s");

...
const childStyle = (child) => {
  const color = OverlayColors.colors[child.colorKey].color;
...
export default {
  addOverlayLayer: function(file, cb) {
    ...
    let addLayer = (_geojson) => {
      cb(null, this.buildOverlay(_geojson, file.name), _geojson);
    }
  },

  buildOverlay: function(geojson, filename, opts = {}) {
    const entry = {
      id: ++overlayId,
    ...
    Object.keys(groups).sort().forEach(layerName => {
      ...
      const child = {
        ...
        colorKey: OverlayColors.colors[savedColor] !== undefined ? savedColor : OverlayColors.nextColorKey(),
        ...
        onEachFeature: (feature, layer) => {
          ...
          const lazyrender = () => {
            ...
            ReactDOM.render(<OverlayFeaturePopup child={child}
              feature={feature}
              onColorChange={colorKey => this.updateOverlayColor(child, colorKey)} />, root);
            return root;
          };
    ...
    this.recomputeOverlayBounds(entry);

    return entry;
  },

  recomputeOverlayBounds: function(entry) {
  ...
  updateOverlayColor: function(child, colorKey) {
    child.colorKey = colorKey;
    const color = OverlayColors.colors[colorKey].color;
  ...
  updateOverlayOpacity: function(entry, opacity) {
